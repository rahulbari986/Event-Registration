...
const sentEmailsDir = path.join(__dirname, 'sent-emails');
...
async function startServer() {
  try {
    ...
    // 2. Attach DB to request object middleware
    app.use((req, res, next) => {
      req.db = db;
      next();
    });

    // 3. Body parsers & session configuration
    ...
    app.use('/generated-cards', express.static(path.join(__dirname, 'generated-cards')));
    // Serve public files statically
    app.use(express.static(path.join(__dirname, 'public')));

    // 6. Vanity Routes (clean URLs without .html)
    app.get('/', (req, res) => {
